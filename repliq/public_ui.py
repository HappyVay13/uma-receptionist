...
(function(){
  ...
  function setLanguage(lang){
    ...
    document.cookie='repliq_ui_lang='+encodeURIComponent(lang)+'; Path=/; Max-Age=31536000; SameSite=Lax'+secure;
    const url=new URL(location.href);url.searchParams.set('ui_lang',lang);location.href=url.toString();
  }
  document.querySelectorAll('[data-rp-lang]').forEach(btn=>btn.addEventListener('click',()=>setLanguage(btn.dataset.rpLang)));
  const button=document.querySelector('[data-rp-menu]');const nav=document.querySelector('[data-rp-mobile-nav]');
  if(button&&nav){button.addEventListener('click',()=>{const open=nav.classList.toggle('open');button.setAttribute('aria-expanded',open?'true':'false');});}
  document.querySelectorAll('a[href^="#"]').forEach(a=>a.addEventListener('click',()=>{if(nav)nav.classList.remove('open');}));
  window.RepliqPublic={
    setLanguage,
...
def _language_buttons(lang: str) -> str:
    return "".join(
        f'<button type="button" class="{"active" if code == lang else ""}" data-rp-lang="{code}" aria-label="{html.escape(public_text(lang, "nav.language"))}: {code.upper()}">{code.upper()}</button>'
...
def render_public_shell(
    *,
    ...
    lang_buttons = _language_buttons(normalized)
...
<div class="rp-actions"><div class="rp-lang" aria-label="{html.escape(public_text(normalized,"nav.language"))}">{lang_buttons}</div><a class="rp-button rp-button-ghost" href="/owner/login?ui_lang={normalized}">{html.escape(public_text(normalized,"nav.login"))}</a><a class="rp-button rp-button-primary" href="/public/signup?ui_lang={normalized}">{html.escape(public_text(normalized,"nav.signup"))}</a><button class="rp-menu" type="button" data-rp-menu aria-expanded="false" aria-label="{html.escape(public_text(normalized,"nav.open"))}">☰</button></div></div>
<nav class="rp-mobile-nav rp-container" data-rp-mobile-nav><a href="/#product">{html.escape(public_text(normalized,"nav.product"))}</a><a href="/#how">{html.escape(public_text(normalized,"nav.how"))}</a><a href="/#security">{html.escape(public_text(normalized,"nav.security"))}</a><a href="/support?ui_lang={normalized}">{html.escape(public_text(normalized,"nav.support"))}</a><a href="/owner/login?ui_lang={normalized}">{html.escape(public_text(normalized,"nav.login"))}</a><a href="/public/signup?ui_lang={normalized}">{html.escape(public_text(normalized,"nav.signup"))}</a></nav></header>'''
...
<section class="rp-section"><div class="rp-container"><div class="rp-cta"><div><h2>{_esc_text(l,"home.final_title")}</h2><p>{_esc_text(l,"home.final_text")}</p></div><a class="rp-button rp-button-secondary" href="/public/signup?ui_lang={l}">{_esc_text(l,"home.cta")}</a></div></div></section>'''
    return render_public_shell(title=public_text(l,"meta.home"), description=public_text(l,"meta.home_desc"), lang=l, content_html=content, active="product")


def render_signup_page(lang: str, readiness: Dict[str, Any]) -> str:
    l = normalize_ui_language(lang)
    ...
    return render_public_shell(title=public_text(l,"meta.signup"), description=public_text(l,"signup.lead"), lang=l, content_html=content, active="signup", inline_script=script)


def render_login_page(lang: str, tenant_id: str, next_path: str) -> str:
    l = normalize_ui_language(lang)
    ...
    return render_public_shell(title=public_text(l,"meta.login"), description=public_text(l,"login.lead"), lang=l, content_html=content, active="login", inline_script=script)


def render_magic_login_page(lang: str, tenant_id: str, next_path: str) -> str:
    l = normalize_ui_language(lang)
    ...
    return render_public_shell(title=public_text(l,"meta.magic"), description=public_text(l,"magic.lead"), lang=l, content_html=content, active="login", inline_script=script)


def render_logout_page(lang: str) -> str:
    l = normalize_ui_language(lang)
    ...
    return render_public_shell(title=public_text(l,"logout.title"), description=public_text(l,"logout.text"), lang=l, content_html=content, active="login")


def render_legal_page(lang: str, kind: str) -> str:
    l = normalize_ui_language(lang)
    if kind == "privacy":
    ...
    return render_public_shell(title=public_text(l,title_key),description=public_text(l,f"{prefix}.lead"),lang=l,content_html=content,active="")


def render_contact_page(lang: str, contact_email: str = "") -> str:
    ...
    return render_public_shell(title=public_text(l,"meta.contact"),description=public_text(l,"contact.lead"),lang=l,content_html=content)


def render_support_page(lang: str) -> str:
    l=normalize_ui_language(lang)
    ...
    return render_public_shell(title=public_text(l,"meta.support"),description=public_text(l,"support.lead"),lang=l,content_html=content,active="support")
