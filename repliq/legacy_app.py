...
# -------------------------
# STAGE 23 — HUMAN CONVERSATIONAL LAYER
# -------------------------
def _stage23_clean_reply(value: Any) -> str:
...
def _stage23_set_reply(result: Dict[str, Any], text_value: str) -> Dict[str, Any]:
    text_value = _stage23_clean_reply(text_value)
    if not text_value:
        return result
    result["msg_out"] = text_value
    result["reply_voice"] = text_value
    result["stage23_humanized"] = True
    return result


def _stage23_has_real_datetime_context(c: Dict[str, Any], pending: Dict[str, Any]) -> bool:
    pending = pending or {}
    if str(c.get("datetime_iso") or "").strip():
        return True
    for key in ("candidate_datetime_iso", "awaiting_time_date_iso", "confirm_slot_iso"):
        if str(pending.get(key) or "").strip():
            return True
    return False


def _stage23_when_from_context(c: Dict[str, Any], pending: Dict[str, Any]) -> str:
    pending = pending or {}
    for key in ("confirm_slot_iso", "candidate_datetime_iso", "datetime_iso", "awaiting_time_date_iso"):
        value = pending.get(key) if key != "datetime_iso" else (c.get("datetime_iso") or pending.get("datetime_iso"))
...
def _stage23_service_from_context(c: Dict[str, Any], pending: Dict[str, Any], lang: str, tenant: Dict[str, Any]) -> str:
    pending = pending or {}
    value = str(pending.get("service_display") or "").strip()
    if value:
        return value
    try:
        catalog = tenant_service_catalog(tenant)
        item = get_service_item_by_key(catalog, c.get("service") or pending.get("service"))
        value = service_display_name(item, lang)
        if value:
            return value
    except Exception:
        pass
    return str(c.get("service") or pending.get("service") or "").strip()


def _stage23_reply_pool(kind: str, lang: str, **kwargs) -> List[str]:
    lang = get_lang(lang)
    service = str(kwargs.get("service") or "").strip()
    when = str(kwargs.get("when") or "").strip()
    slots = kwargs.get("slots") or []
    joined_slots = ", ".join(slots[:3]) if isinstance(slots, list) else str(slots or "")

    pools: Dict[str, Dict[str, List[str]]] = {
        "ask_service": {
            "lv": [
                "Protams. Uz kādu pakalpojumu vēlaties pierakstīties?",
                "Labi, paskatīsimies. Kuru pakalpojumu vēlaties?",
                "Skaidrs. Pasakiet, lūdzu, kāds pakalpojums nepieciešams.",
                "Jā, varam palīdzēt. Uz kuru pakalpojumu pierakstām?",
            ],
            "ru": [
                "Конечно. На какую услугу вас записать?",
                "Хорошо, посмотрим. Какая услуга нужна?",
                "Понял. Подскажите, на какую услугу записать?",
                "Да, помогу. Что именно хотите сделать?",
            ],
            "en": [
                "Of course. Which service would you like to book?",
                "Sure, let’s check. What service do you need?",
                "Got it. What would you like to book?",
            ],
        },
        "ask_service_with_time": {
            "lv": [
                f"Laiku piefiksēju: {when}. Uz kādu pakalpojumu vēlaties pierakstīties?",
                f"Skaidrs, {when} piefiksēju. Kāds pakalpojums nepieciešams?",
                f"Labi, par {when} sapratu. Uz kuru pakalpojumu pierakstām?",
            ],
            "ru": [
                f"Время понял: {when}. На какую услугу вас записать?",
                f"Хорошо, {when} зафиксировал. Какая услуга нужна?",
                f"Понял по времени — {when}. Что именно хотите сделать?",
            ],
            "en": [
                f"Got it — {when}. Which service would you like to book?",
                f"Sure, I noted {when}. What service do you need?",
                f"Okay, {when} works as the requested time. What would you like to book?",
            ],
        },
        "ask_date": {
            "lv": [
                f"Labi{', ' + service if service else ''}. Uz kuru dienu vēlaties pierakstīties?",
                f"Skaidrs{', ' + service if service else ''}. Kurš datums jums būtu ērtāks?",
                f"Protams. Kuru dienu skatāmies{(' priekš ' + service) if service else ''}?",
            ],
            "ru": [
                f"Хорошо{', ' + service if service else ''}. На какой день вас записать?",
                f"Понял{', ' + service if service else ''}. Какая дата вам удобна?",
                f"Конечно. На какой день смотрим время?",
            ],
            "en": [
                f"Sure{', ' + service if service else ''}. Which day would work for you?",
                f"Got it{', ' + service if service else ''}. What date would you prefer?",
            ],
        },
        "ask_time": {
            "lv": [
                f"Skaidrs{', ' + when if when else ''}. Kāds laiks jums būtu ērtāks?",
                f"Labi{', ' + when if when else ''}. Cikos jums derētu?",
                f"Protams. Kādu laiku tajā dienā skatāmies?",
            ],
            "ru": [
                f"Понял{', ' + when if when else ''}. Какое время вам удобно?",
                f"Хорошо{', ' + when if when else ''}. На сколько вас записать?",
                f"Конечно. Какое время в этот день подойдёт?",
            ],
            "en": [
                f"Got it{', ' + when if when else ''}. What time would work for you?",
                f"Sure{', ' + when if when else ''}. What time should I check?",
            ],
        },
        "offer_slots": {
            "lv": [
                f"Varu piedāvāt šos laikus: {joined_slots}. Kurš jums der?",
                f"Ir pieejami šādi varianti: {joined_slots}. Kuru izvēlamies?",
                f"Brīvie laiki ir: {joined_slots}. Kurš būtu ērtāks?",
            ],
            "ru": [
                f"Могу предложить такие варианты: {joined_slots}. Какой вам удобнее?",
                f"Есть свободные варианты: {joined_slots}. Что выбираем?",
                f"Свободно так: {joined_slots}. Какое время подойдёт?",
            ],
            "en": [
                f"I can offer these times: {joined_slots}. Which one works best?",
                f"These times are available: {joined_slots}. Which would you prefer?",
            ],
        },
        "confirm": {
            "lv": [
                f"Tad apstiprinām pierakstu{(' uz ' + service) if service else ''}{(' — ' + when) if when else ''}?",
                f"Vai apstiprināt{(' ' + service) if service else ''}{(' uz ' + when) if when else ''}?",
                f"Labi, tad varu apstiprināt{(' ' + service) if service else ''}{(' uz ' + when) if when else ''}?",
            ],
            "ru": [
                f"Тогда подтверждаем запись{(' на ' + service) if service else ''}{(' — ' + when) if when else ''}?",
                f"Подтверждаем{(' ' + service) if service else ''}{(' на ' + when) if when else ''}?",
                f"Хорошо, могу подтвердить запись{(' на ' + when) if when else ''}?",
            ],
            "en": [
                f"Shall I confirm the booking{(' for ' + service) if service else ''}{(' at ' + when) if when else ''}?",
                f"Would you like me to confirm{(' ' + service) if service else ''}{(' for ' + when) if when else ''}?",
            ],
        },
        "unclear": {
            "lv": [
                "Precizējiet, lūdzu — vēlaties pierakstīties vai uzzināt informāciju?",
                "Sapratu. Lai nepārprastu — jūs vēlaties pierakstu vai vienkārši informāciju?",
                "Varat, lūdzu, mazliet precizēt, ko tieši vēlaties?",
            ],
            "ru": [
                "Уточните, пожалуйста — хотите записаться или просто узнать информацию?",
                "Понял. Чтобы не ошибиться: нужна запись или информация?",
                "Можете чуть точнее написать, что именно нужно?",
            ],
            "en": [
                "Could you clarify — would you like to book, or do you just need information?",
                "Got it. Just to make sure: do you want an appointment or some information?",
            ],
        },
    }
    return pools.get(kind, {}).get(lang) or pools.get(kind, {}).get("lv") or []


def _stage23_choose(kind: str, lang: str, previous_text: str = "", **kwargs) -> str:
    options = _stage23_reply_pool(kind, lang, **kwargs)
    if not options:
        return ""
    prev_norm = _normalize_phrase_text(previous_text)
    filtered = [x for x in options if _normalize_phrase_text(x) != prev_norm]
    return _pick_variant(filtered or options, options[0])


def stage23_human_conversational_layer(
    result: Dict[str, Any],
    ...
    """Lightweight receptionist-style response layer.

    This is intentionally deterministic/cheap: it does not call the LLM.
    It only rewrites safe clarification/confirmation surfaces while preserving
    the underlying orchestration, state machine, calendar and FAQ behavior.
    """
    result = dict(result or {})
    conv = conv or {}
    tenant = tenant or {}
    pending = conv.get("pending") or {}
    lang = get_lang(result.get("lang") or conv.get("lang") or "lv")
    state = conversation_state(conv)
    status = str(result.get("status") or "").strip().lower()
    original = _stage23_clean_reply(result.get("msg_out") or result.get("reply_voice") or "")

    if status in {"blocked", "cancelled", "cancel_failed", "booking_failed", "no_booking"}:
        return result
    if not original and status not in {"need_more", "busy"}:
        return result

    service = _stage23_service_from_context(conv, pending, lang, tenant)
    when = _stage23_when_from_context(conv, pending)
    slots = _slot_labels_from_pending(pending)

    # Do not rewrite factual FAQ answers unless the flow explicitly continues.
    if status == "info" and not result.get("flow_preserved"):
        result["preserve_text"] = True
        return result

    # Humanize only the safe surfaces: missing-field prompts, slot offers and confirmations.
    if status in {"need_more", "busy", "greeting"}:
        if state == STATE_AWAITING_SERVICE:
            if when:
                return _stage23_set_reply(result, _stage23_choose("ask_service_with_time", lang, original, when=when))
            return _stage23_set_reply(result, _stage23_choose("ask_service", lang, original))

        if state == STATE_AWAITING_DATE:
            return _stage23_set_reply(result, _stage23_choose("ask_date", lang, original, service=service))

        if state == STATE_AWAITING_TIME:
            if slots:
                return _stage23_set_reply(result, _stage23_choose("offer_slots", lang, original, slots=slots))
            return _stage23_set_reply(result, _stage23_choose("ask_time", lang, original, when=when))

        if state == STATE_AWAITING_CONFIRM:
            return _stage23_set_reply(result, _stage23_choose("confirm", lang, original, service=service, when=when))

    # Last safe clean-up for very robotic generic fallback.
    low_original = _normalize_phrase_text(original)
    if status in {"need_more", "recovery"} and low_original in {
        "ka varu palidzet", "kā varu palīdzēt", "how can i help", "чем могу помочь"
    }:
        return _stage23_set_reply(result, _stage23_choose("unclear", lang, original))
...
def handle_user_text_with_logging(
    ...
    try:
        ...
        result = stage23_human_conversational_layer(result, conv, tenant, raw_text=text_in, channel=channel)
